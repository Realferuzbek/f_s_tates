...
- When deploying to Vercel, include the provided `vercel.json` and root `package.json`. They instruct Vercel to install from `frontend/`, emit the Vite build into `frontend/dist`, and serve it as a single-page app without intercepting hashed asset requests. Set `VITE_API_BASE_URL` in your Vercel environment variables so the client knows where to reach the running backend (e.g., `https://your-backend-host.com/api`).
